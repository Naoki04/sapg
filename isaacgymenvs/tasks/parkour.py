...
from isaacgymenvs.tasks.terrain.terrain import Terrain
from isaacgymenvs.tasks.utils.math import *
from isaacgymenvs.tasks.utils.helpers import class_to_dict
...
class Parkour(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cfg['env']['numObservations'] = self.cfg['env']['num_observations']
        self.lookat_id = 0
        ...
        self.resize_transform = torchvision.transforms.Resize((self.cfg['env']['depth']['resized'][1], self.cfg['env']['depth']['resized'][0]), 
    ...
    def step(self, actions):
        ...
        if self.cfg['env']['domain_rand']['action_delay']:
            if self.global_counter % self.cfg['env']['domain_rand']['delay_update_global_steps'] == 0:
                if len(self.cfg['env']['domain_rand']['action_curr_step']) != 0:
                    self.delay = torch.tensor(self.cfg['env']['domain_rand']['action_curr_step'].pop(0), device=self.device, dtype=torch.float)
            if self.viewer:
                self.delay = torch.tensor(self.cfg['env']['domain_rand']['action_delay_view'], device=self.device, dtype=torch.float)
            indices = -self.delay - 1
            actions = self.action_history_buf[:, indices.long()]  # delay for 1/50=20ms

        self.global_counter += 1
        self.total_env_steps_counter += 1
        clip_actions = self.cfg['env']['normalization']['clip_actions'] / self.cfg['env']['control']['action_scale']
        self.actions = torch.clip(actions, -clip_actions, clip_actions).to(self.device)
        self.render()

        for _ in range(self.cfg['env']['control']['decimation']):
        ...
        clip_obs = self.cfg['env']['normalization']['clip_observations']
        self.obs_buf = torch.clip(self.obs_buf, -clip_obs, clip_obs)
        # if self.privileged_obs_buf is not None:
        #     self.privileged_obs_buf = torch.clip(self.privileged_obs_buf, -clip_obs, clip_obs)
        self.extras["delta_yaw_ok"] = self.delta_yaw < 0.6
        if self.cfg['env']['depth']['use_camera'] and self.global_counter % self.cfg['env']['depth']['update_interval'] == 0:
            self.extras["depth"] = self.depth_buffer[:, -2]  # have already selected last one
        else:
            self.extras["depth"] = None

        self.extras["true_objective"] = self.get_true_objective()
    ...
    def get_true_objective(self):
        dis_to_origin = torch.norm(self.root_states[:, :2] - self.env_origins[:, :2], dim=1)
        threshold = self.commands[:, 0] * self.cfg['env']['episode_length_s']
        return dis_to_origin / threshold
    ...
    def update_depth_buffer(self):
        if not self.cfg['env']['depth']['use_camera']:
            return
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.enable_viewer_sync and self.debug_viz:
            ...
            # self._draw_goals()
            self._draw_feet()
            if self.cfg['env']['depth']['use_camera']:
    ...
    def check_termination(self):
        ...
        # reach_goal_cutoff = self.cur_goal_idx >= self.cfg['env']['terrain']['num_goals']
    ...
    def reset_idx(self, env_ids):
        ...
        if len(env_ids) == 0:
            return
        # update curriculum
        if self.cfg['env']['terrain']['curriculum']:
            self._update_terrain_curriculum(env_ids)
        # avoid updating command curriculum at each step since the maximum command is common to all envs
        if self.cfg['env']['commands']['curriculum'] and (self.common_step_counter % self.max_episode_length == 0):
        ...
        if self.cfg['env']['commands']['curriculum']:
    ...
    def compute_reward(self):
        ...
        if self.cfg['env']['rewards']['only_positive_rewards']:
    ...
    def compute_observations(self):
        ...
        if self.global_counter % 5 == 0:
            self.delta_yaw = torch.zeros_like(self.yaw, device=self.device)
            self.delta_next_yaw = torch.zeros_like(self.yaw, device=self.device)
    ...
    def get_noisy_measurement(self, x, scale):
        if self.cfg['env']['noise']['add_noise']:
            x = x + (2.0 * torch.rand_like(x) - 1) * scale * self.cfg['env']['noise']['noise_level']
        return x
    ...
    def create_sim(self):
        ...
        mesh_type = self.cfg['env']['terrain']['mesh_type']
        start = time()
        print("*"*80)
    ...
    def _process_rigid_shape_props(self, props, env_id):
        ...
        """
        if self.cfg['env']['domain_rand']['randomize_friction']:
            if env_id==0:
                # prepare friction randomization
                friction_range = self.cfg['env']['domain_rand']['friction_range']
                num_buckets = 64
    ...
    def _process_dof_props(self, props, env_id):
        ...
        if env_id==0:
            ...
            for i in range(len(props)):
                ...
                self.dof_pos_limits[i, 0] = m - 0.5 * r * self.cfg['env']['rewards']['soft_dof_pos_limit']
                self.dof_pos_limits[i, 1] = m + 0.5 * r * self.cfg['env']['rewards']['soft_dof_pos_limit']
        return props

    def _process_rigid_body_props(self, props, env_id):
        # No need to use tensors as only called upon env creation
        if self.cfg['env']['domain_rand']['randomize_base_mass']:
            rng_mass = self.cfg['env']['domain_rand']['added_mass_range']
            rand_mass = np.random.uniform(rng_mass[0], rng_mass[1], size=(1, ))
            props[0].mass += rand_mass
        else:
            rand_mass = np.zeros((1, ))
        if self.cfg['env']['domain_rand']['randomize_base_com']:
            rng_com = self.cfg['env']['domain_rand']['added_com_range']
    ...
    def _post_physics_step_callback(self):
        ...
        """
        # 
        env_ids = (self.episode_length_buf % int(self.cfg['env']['commands']['resampling_time'] / self.dt) == 0)
        self._resample_commands(env_ids.nonzero(as_tuple=False).flatten())

        if self.cfg['env']['commands']['heading_command']:
            forward = quat_apply(self.base_quat, self.forward_vec)
            heading = torch.atan2(forward[:, 1], forward[:, 0])
            self.commands[:, 2] = torch.clip(0.8 * wrap_to_pi(self.commands[:, 3] - heading), -1., 1.)
            self.commands[:, 2] *= torch.abs(self.commands[:, 2]) > self.cfg['env']['commands']['ang_vel_clip']
        
        if self.cfg['env']['terrain']['measure_heights']:
            if self.global_counter % self.cfg['env']['depth']['update_interval'] == 0:
                self.measured_heights = self._get_heights()
        if self.cfg['env']['domain_rand']['push_robots'] and (self.common_step_counter % self.cfg['env']['domain_rand']['push_interval'] == 0):
            self._push_robots()
    ...
    def _resample_commands(self, env_ids):
        ...
        if self.cfg['env']['commands']['heading_command']:
            ...
            self.commands[env_ids, 2] *= torch.abs(self.commands[env_ids, 2]) > self.cfg['env']['commands']['ang_vel_clip']

        # set small commands to zero
        self.commands[env_ids, :2] *= torch.abs(self.commands[env_ids, 0:1]) > self.cfg['env']['commands']['lin_vel_clip']
    ...
    def _compute_torques(self, actions):
        ...
        """
        #pd controller
        actions_scaled = actions * self.cfg['env']['control']['action_scale']
        control_type = self.cfg['env']['control']['control_type']
        if control_type=="P":
            if not self.cfg['env']['domain_rand']['randomize_motor']:  # TODO add strength to gain directly
    ...
    def _push_robots(self):
        ...
        """
        max_vel = self.cfg['env']['domain_rand']['max_push_vel_xy']
    ...
    def _init_buffers(self):
        ...
        str_rng = self.cfg['env']['domain_rand']['motor_strength_range']
        ...
        self.action_history_buf = torch.zeros(self.num_envs, self.cfg['env']['domain_rand']['action_buf_len'], self.num_dofs, device=self.device, dtype=torch.float)
        self.contact_buf = torch.zeros(self.num_envs, self.cfg['env']['contact_buf_len'], 4, device=self.device, dtype=torch.float)

        self.commands = torch.zeros(self.num_envs, self.cfg['env']['commands']['num_commands'], dtype=torch.float, device=self.device, requires_grad=False) # x vel, y vel, yaw vel, heading
        ...
        for i in range(self.num_dofs):
            ...
            angle = self.cfg['env']['init_state']['default_joint_angles'][name]
            self.default_dof_pos[i] = angle
            found = False
            for dof_name in self.cfg['env']['control']['stiffness'].keys():
                if dof_name in name:
                    self.p_gains[i] = self.cfg['env']['control']['stiffness'][dof_name]
                    self.d_gains[i] = self.cfg['env']['control']['damping'][dof_name]
                    found = True
            if not found:
                self.p_gains[i] = 0.
                self.d_gains[i] = 0.
        ...
        if hasattr(self.cfg['env'], "height_update_dt"):
            self.height_update_interval = int(self.cfg['env']['height_update_dt'] / (self.cfg['sim']['dt'] * self.cfg['env']['control']['decimation']))

        if self.cfg['env']['depth']['use_camera']:
            self.depth_buffer = torch.zeros(self.num_envs,  
                            self.cfg['env']['depth']['buffer_len'], 
                            self.cfg['env']['depth']['resized'][1], 
                            self.cfg['env']['depth']['resized'][0]).to(self.device)
    ...
    def _create_trimesh(self):
        ...
        tm_params.static_friction = self.cfg['env']['terrain']['static_friction']
        tm_params.dynamic_friction = self.cfg['env']['terrain']['dynamic_friction']
    ...
    def attach_camera(self, i, env_handle, actor_handle):
        if self.cfg['env']['depth']['use_camera']:
            config = self.cfg['env']['depth']
            camera_props = gymapi.CameraProperties()
            camera_props.width = self.cfg['env']['depth']['original'][0]
            camera_props.height = self.cfg['env']['depth']['original'][1]
            camera_props.enable_tensors = True
            camera_horizontal_fov = self.cfg['env']['depth']['horizontal_fov']
    ...
    def _create_envs(self):
        ...
        assets_root_path = os.path.join(project_root_path, 'assets')
        asset_path = self.cfg['env']['asset']['file'].format(LEGGED_GYM_ROOT_DIR=assets_root_path)
        asset_root = os.path.dirname(asset_path)
        asset_file = os.path.basename(asset_path)

        asset_options = gymapi.AssetOptions()
        asset_options.default_dof_drive_mode = self.cfg['env']['asset']['default_dof_drive_mode']
        asset_options.collapse_fixed_joints = self.cfg['env']['asset']['collapse_fixed_joints']
        asset_options.replace_cylinder_with_capsule = self.cfg['env']['asset']['replace_cylinder_with_capsule']
        asset_options.flip_visual_attachments = self.cfg['env']['asset']['flip_visual_attachments']
        asset_options.fix_base_link = self.cfg['env']['asset']['fix_base_link']
        asset_options.density = self.cfg['env']['asset']['density']
        asset_options.angular_damping = self.cfg['env']['asset']['angular_damping']
        asset_options.linear_damping = self.cfg['env']['asset']['linear_damping']
        asset_options.max_angular_velocity = self.cfg['env']['asset']['max_angular_velocity']
        asset_options.max_linear_velocity = self.cfg['env']['asset']['max_linear_velocity']
        asset_options.armature = self.cfg['env']['asset']['armature']
        asset_options.thickness = self.cfg['env']['asset']['thickness']
        asset_options.disable_gravity = self.cfg['env']['asset']['disable_gravity']
        ...
        feet_names = [s for s in body_names if self.cfg['env']['asset']['foot_name'] in s]
        ...
        for i in tqdm(range(self.num_envs)):
            ...
            anymal_handle = self.gym.create_actor(env_handle, robot_asset, start_pose, "anymal", i, self.cfg['env']['asset']['self_collisions'], 0)
    ...
    def _get_env_origins(self):
        ...
        """
        if self.cfg['env']['terrain']['mesh_type'] in ["heightfield", "trimesh"]:
            self.custom_origins = True
            self.env_origins = torch.zeros(self.num_envs, 3, device=self.device, requires_grad=False)
            # put robots at the origins defined by the terrain
            max_init_level = self.cfg['env']['terrain']['max_init_terrain_level']
            if not self.cfg['env']['terrain']['curriculum']: 
                max_init_level = self.cfg['env']['terrain']['num_rows'] - 1
            self.terrain_levels = torch.randint(0, max_init_level+1, (self.num_envs,), device=self.device)
            

            self.terrain_types = torch.div(torch.arange(self.num_envs, device=self.device), (self.num_envs/self.cfg['env']['terrain']['num_cols']), rounding_mode='floor').to(torch.long)
            self.max_terrain_level = self.cfg['env']['terrain']['num_rows']
            self.terrain_origins = torch.from_numpy(self.terrain.env_origins).to(self.device).to(torch.float)

            # random assign
            self.terrain_levels = torch.randint(0, self.cfg['env']['terrain']['num_rows'], (self.num_envs,), device=self.device)
            # set terrain levels to 0
            # self.terrain_levels[:] = 0
            # self.terrain_types = torch.randint(0, self.cfg.terrain.num_cols, (self.num_envs,), device=self.device)
            # print("i, j:", self.terrain_levels, self.terrain_types)
            # assert False
    ...
    def _parse_cfg(self, cfg):
        self.dt = self.cfg['env']["control"]['decimation'] * self.cfg['sim']["dt"]
        self.obs_scales = self.cfg['env']['normalization']['obs_scales']
        self.reward_scales = self.cfg['env']['rewards']['scales']
        ...
        if self.cfg['env']['commands']['curriculum']:
            self.command_ranges = self.cfg['env']['commands']['ranges']
        else:
            self.command_ranges = self.cfg['env']['commands']['max_ranges']
        if self.cfg['env']['terrain']['mesh_type'] not in ['heightfield', 'trimesh']:
            self.cfg['env']['terrain']['curriculum'] = False
        self.max_episode_length_s = self.cfg['env']['episode_length_s']
        self.max_episode_length = np.ceil(self.max_episode_length_s / self.dt)

        self.cfg['env']['domain_rand']['push_interval'] = np.ceil(self.cfg['env']['domain_rand']['push_interval_s'] / self.dt)
    ...
    def _init_height_points(self):
        ...
        """
        y = torch.tensor(self.cfg['env']['terrain']['measured_points_y'], device=self.device, requires_grad=False)
        x = torch.tensor(self.cfg['env']['terrain']['measured_points_x'], device=self.device, requires_grad=False)
        grid_x, grid_y = torch.meshgrid(x, y)

        self.num_height_points = grid_x.numel()
        points = torch.zeros(self.num_envs, self.num_height_points, 3, device=self.device, requires_grad=False)
        for i in range(self.num_envs):
            offset = torch_rand_float(-self.cfg['env']['terrain']['measure_horizontal_noise'], self.cfg['env']['terrain']['measure_horizontal_noise'], (self.num_height_points, 2), device=self.device).squeeze()
            xy_noise = torch_rand_float(-self.cfg['env']['terrain']['measure_horizontal_noise'], self.cfg['env']['terrain']['measure_horizontal_noise'], (self.num_height_points, 2), device=self.device).squeeze() + offset
    ...
    def _get_heights(self, env_ids=None):
        ...
        Returns:
            [type]: [description]
        """
        if self.cfg['env']['terrain']['mesh_type'] == 'plane':
            return torch.zeros(self.num_envs, self.num_height_points, device=self.device, requires_grad=False)
        elif self.cfg['env']['terrain']['mesh_type'] == 'none':
    ...
    def _reward_feet_edge(self):
        feet_pos_xy = ((self.rigid_body_states[:, self.feet_indices, :2] + self.terrain.cfg['border_size']) / self.cfg['env']['terrain']['horizontal_scale']).round().long()  # (num_envs, 4, 2)
